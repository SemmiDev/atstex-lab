...
const cmTheme = EditorView.theme(
  {
    ...
    "&.cm-focused .cm-selectionBackground, ::selection": {
      backgroundColor: "rgba(255, 71, 148, 0.2)",
    },
  },
  { dark: false },
);
...
const latexCompletions = [
  ...
  snippetCompletion("\\sum_{${lower}}^{${upper}}", {
    label: "sum",
    info: "Summation",
  }),
  snippetCompletion("\\int_{${lower}}^{${upper}}", {
    label: "int",
    info: "Integral",
  }),
...
function showLog(raw, success) {
  raw =
    raw ||
    (success
      ? "Dokumen berhasil diproses tanpa pesan log tambahan."
  ...
  logOutput.innerHTML = raw
    ...
    .map((line) => {
      ...
      if (/warning/i.test(line)) return `<span class="warn-line">${esc}</span>`;
      return esc;
    })
    .join("\n");
  ...
  if (!success) {
    // Only auto-open the log panel if there was an error
...
function getPageSettings() {
  return {
    ...
    lineSpacing:
      parseFloat(document.getElementById("ps-line-spacing").value) || 1.0,
...
function setPageSettings(s) {
  if (!s) return;
  if (s.documentClass)
    document.getElementById("ps-doc-class").value = s.documentClass;
  ...
  if (s.fontFamily)
    document.getElementById("ps-font-family").value = s.fontFamily;
  if (s.marginTop) document.getElementById("ps-margin-top").value = s.marginTop;
  if (s.marginBottom)
    document.getElementById("ps-margin-bottom").value = s.marginBottom;
  if (s.marginLeft)
    document.getElementById("ps-margin-left").value = s.marginLeft;
  if (s.marginRight)
    document.getElementById("ps-margin-right").value = s.marginRight;
  if (s.lineSpacing)
    document.getElementById("ps-line-spacing").value = String(s.lineSpacing);
  if (s.alignment) document.getElementById("ps-alignment").value = s.alignment;
  if (s.headerText !== undefined)
    document.getElementById("ps-header-text").value = s.headerText;
  if (s.footerText !== undefined)
    document.getElementById("ps-footer-text").value = s.footerText;
}
...
const previewErrorPanel = document.getElementById("preview-error-panel");
const previewErrorBody = document.getElementById("preview-error-body");
const previewErrorClose = document.getElementById("preview-error-close");

// Hide error panel on close button click
if (previewErrorClose) {
  previewErrorClose.addEventListener("click", () => {
    previewErrorPanel.style.display = "none";
  });
}

function showPreviewError(msg) {
  if (!previewErrorPanel || !previewErrorBody) return;
  previewErrorBody.textContent = msg;
  previewErrorPanel.style.display = "";
}

function hidePreviewError() {
  if (previewErrorPanel) previewErrorPanel.style.display = "none";
}
...
async function syncTemplateContent() {
  ...
  if (
    ...
    if (renderRes.ok) {
      ...
      // Only dispatch if content actually changed to avoid cursor jumping
