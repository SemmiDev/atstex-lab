...
CONTAINER_NAME="${CONTAINER_NAME:-atstex-lab}"
DB_CONTAINER="${DB_CONTAINER:-atstex-postgres}"
VERBOSE="${1:-}"
...
echo -e "${BLUE}╔══════════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║  ATSTEX-LAB — Health Check                   ║${NC}"
echo -e "${BLUE}╚══════════════════════════════════════════════╝${NC}"
echo ""
...
if docker exec "${DB_CONTAINER}" pg_isready -U atstex -d atstex &>/dev/null; then
...
if [[ "${VERBOSE}" == "--verbose" || "${VERBOSE}" == "-v" ]]; then
    ...
    docker images atstex-lab --format "table {{.Tag}}\t{{.CreatedAt}}\t{{.Size}}" 2>/dev/null
fi
